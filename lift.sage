        ...
        for t in f:
            ...
            if (degx - i >= 0) and (degy - (r-i) >= 0) and Not_Zero_Vec(coef):
                ...
                if (b12 % p^(n-r+1) == 1):
                    ...
                    derf.append([ WittProd(v1, [ coef[i] for i in range(0, n+1-r)], pols = pols), degx-i, degy - (r-i)])
...
def lift(a0, b0, n, pols=[], minimal=False):
    ...
    """ Compute the lift of an ordinary elliptic curve C (in Weierstrauss form) over the field k
        of the form 
        
                    C/k : y0 = x0^3 + a0 x0 + b0
    
        Arguments:
            
            a0: Element of the field k
            
            b0: Element of the field k
            
            n: An integer, the length of the ring of Witt vectors to lift to (n >= 1 as n = 0 corresponds to k itself)
        
            pols: See Finotti's notes: https://github.com/lrfinotti/witt/blob/master/README.md
            
            minimal: A Boolean, minimize the degree of the lift
                
                Note: The minimal degree lift is not necessarily the canonical lifting of an elliptic curve.  The
                minimal degree lift extends only to the affine points on curve C and not the point at inifinity. 
            
        Returns:
        
            For the lift CC / W_n(k): y = x^3 + a x + b (where a, b are Witt Vectors) first returns a, then b,
            (as a Witt Vector) then a vector of x in terms of x0 (after applying the Greenburg transform to x),
            then a vector of y such that y_n is the n-th entry times y_0 (again, after applying the Greenburg
            transform to y).
    ...
    for i in range(0, n):
        ...
        vvars = vF + vG
        GTx = GT( [[ vone, 3, 0], [va, 1, 0], [vb, 0, 0]], pols=pols, vvars=vvars) #x^3 + ax + b
        GTy = GT( [[ vone, 0, 2]], pols=pols, vvars=vvars) #y^2
